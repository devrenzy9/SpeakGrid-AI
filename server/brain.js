...
You are a world-class Technical Assistant and your name is SpeakGrid AI. You possess deep, comprehensive knowledge of all things in technology—from computer science fundamentals and software architecture to full-stack web development, ethical hacking, and game design.
...
export async function askBrain(userInput, fileInfo = null, sessionId = "default") {
  try {
    if (!chatSessions.has(sessionId)) {
      ...
      const newChat = model.startChat({
        history: [
          ...
          { role: "model", parts: [{ text: "Hello, I am SpeakGrid AI. I am ready to assist." }] }
        ],
      });
    ...
    if (fileInfo) {
      console.log(`Uploading ${fileInfo.originalname} to SpeakGrid AI...`);
    ...
    // 4. INTERCEPT FUNCTION CALLS (If SpeakGrid AI decides it needs to search the web)
    ...
    if (functionCalls && functionCalls.length > 0) {
      ...
      if (call.name === "search_web") {
        console.log(`[ACTION] SpeakGrid AI is searching the web for: "${call.args.query}"`);
        ...
        const tavilyResponse = await fetch('https://api.tavily.com/search', {
          ...
          body: JSON.stringify({
            ...
            search_depth: "basic",
            max_results: 10 // Keep it small so SpeakGrid AI reads it quickly
          }),
        });

        const searchData = await tavilyResponse.json();

        // Send the raw internet data back to SpeakGrid AI so it can read it and formulate an answer
        console.log("[ACTION] Sending search results back to SpeakGrid AI for summarization...");
...
export async function transformReplyForSpeech(reply, outputMode = "default") {
  const mode = (outputMode || "default").trim().toLowerCase();
  if (!reply || mode === "default") {
    return reply;
  }

  const modeInstructions = {
    concise: "Make it shorter, direct, and easy to speak aloud.",
    teacher: "Explain clearly in a structured but natural spoken way, with simple language.",
    "support-agent": "Use a calm, polite, solution-oriented tone with clear guidance.",
  };
  ...
  try {
    const prompt = `Rewrite the assistant reply for spoken delivery in mode: ${mode}.

Rules:
- Preserve factual meaning and correctness.
- ${selectedInstruction}
- Optimize for natural voice playback.
- Avoid markdown, code blocks, and list-heavy formatting unless naturally spoken.
...
${reply}`;
    ...
    console.error(`Speech transformation failed for mode "${mode}":`, error);
    return reply;
  }
}

export async function translateForTts(text, targetLanguage) {
  const normalizedTarget = (targetLanguage || "Auto").trim();
  if (!text || normalizedTarget.toLowerCase() === "auto") {
    return text;
  }

  try {
    const translationPrompt = `Translate the following assistant reply into ${normalizedTarget}.

Rules:
- Preserve original meaning, conversational tone, and brevity.
- Do not add commentary, notes, explanations, or extra sentences.
...
${text}`;
    ...
    const result = await translationModel.generateContent(translationPrompt);
    ...
    console.error(`Translation failed for target language "${normalizedTarget}":`, error);
    return text;
  }
}
